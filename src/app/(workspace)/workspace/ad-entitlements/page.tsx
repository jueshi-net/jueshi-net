...
export default async function AdEntitlementsPage() {
  ...
  // 查询用户的广告申请 + 右侧栏数据
  const [adApplications, user, unreadNotifs, badgeCount, recentMemos] = await Promise.allSettled([
  ...
  const unreadNotifsCount = unreadNotifs.status === "fulfilled" ? unreadNotifs.value : 0;
  const badgeCountVal = badgeCount.status === "fulfilled" ? badgeCount.value : 0;
  const recentMemosVal = recentMemos.status === "fulfilled" ? recentMemos.value : [];

...
import { FileText, Plus, Edit2, Trash2, Save, X, Loader2, Eye, Globe, Video, AlertCircle } from "lucide-react";

interface Topic {
  id: string; slug: string; title: string; subtitle: string | null;
  status: string; templateType: string; coverEmoji: string | null;
  youtubeUrl: string | null; publishedAt: string | null; createdAt: string; updatedAt: string;
...
export default function AdminTopicsPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      setTopics(data.data || []);
    } catch { setMessage({ type: "error", text: "获取专题列表失败" }); }
  ...
  if (loading) return <div className="p-8 text-gray-500 flex items-center gap-2"><Loader2 className="w-5 h-5 animate-spin" /> 加载中...</div>;
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50 border-b">
            <tr>
              ...
              <th className="text-left px-4 py-3 font-medium text-gray-500">模板</th>
          ...
          <tbody className="divide-y">
            {topics.length === 0 ? (
              <tr><td colSpan={6} className="text-center py-8 text-gray-400">暂无专题，点击上方「新建专题」创建</td></tr>
            ) : topics.map(t => {
              ...
              return (
                <tr key={t.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-gray-500">{t.templateType}</td>
                  <td className="px-4 py-3 hidden md:table-cell">
                    {t.youtubeUrl ? <Video className="w-4 h-4 text-blue-500" /> : <span className="text-gray-300">—</span>}
                  </td>
                  <td className="px-4 py-3 hidden md:table-cell text-gray-500">{t._count.items}</td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1">
                      <Link href={`/topics/${t.slug}`} target="_blank" className="p-1.5 rounded hover:bg-gray-100" title="预览">
                        <Eye className="w-4 h-4 text-gray-400" />
                      </Link>
                      <button onClick={() => startEdit(t)} className="p-1.5 rounded hover:bg-gray-100" title="编辑">

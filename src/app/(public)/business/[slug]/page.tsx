/**
 * /business/[slug] - Business/Organization provider detail page.
 * Only allows providerType=ORGANIZATION or OFFICIAL, status=APPROVED.
 */
...
import { providerMetadata, organizationJsonLd, breadcrumbJsonLd } from "@/modules/service-provider/ui/seo";
...
export default async function BusinessDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!provider || (provider.providerType !== "ORGANIZATION" && provider.providerType !== "OFFICIAL")) {
    notFound();
  }
  ...
  const jsonLdOrg = organizationJsonLd(provider);
  const jsonLdBreadcrumbs = breadcrumbJsonLd([
    { name: "首页", url: "/" },
    { name: "服务商", url: "/service-providers" },
    { name: provider.displayName, url: `/business/${provider.slug}` },
  ]);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* Cover */}
      {provider.coverUrl && (
        <div className="h-32 w-full overflow-hidden bg-gray-200 sm:h-48">
          <img src={provider.coverUrl} alt="" className="h-full w-full object-cover" />
        </div>
      )}
            ...
            <div className="rounded-xl border border-gray-200 bg-white p-5">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-1 flex flex-wrap items-center gap-2">
                    <span className="rounded bg-blue-50 px-2 py-0.5 text-xs font-medium text-blue-700">
                      {provider.providerType === "OFFICIAL" ? "平台官方" : "企业"}
                    </span>
                    {provider.isVerified && (
              ...
              {provider.serviceAreas.length > 0 && (
                <div className="mt-4 border-t border-gray-100 pt-3">
                  <h3 className="mb-1 text-xs font-semibold uppercase tracking-wide text-gray-500">服务地区</h3>
                  <div className="flex flex-wrap gap-1.5">
                    {provider.serviceAreas.map((a) => (
                      <span key={a} className="rounded bg-gray-100 px-2 py-0.5 text-xs text-gray-600">{a}</span>
                    ))}
                  </div>
                </div>
              )}
              ...
              <div className="mt-4 flex gap-2">
                <ActionButton
                  ...
                  context={{ providerId: provider.id, providerSlug: provider.slug, providerName: provider.displayName, sourceType: "business_detail", sourceId: provider.slug }}
                  label="联系咨询"
                  variant="primary"
                />
                <ActionButton
                      ...
                      <div className="flex items-start justify-between">
                        <div className="min-w-0 flex-1">
                          ...
                          <div className="mt-1 flex flex-wrap gap-1">
                            ...
                            <span className="rounded bg-gray-50 px-1.5 py-0.5 text-xs text-gray-500">{s.deliveryMode}</span>
                        ...
                        <ActionButton
                          action="service.request"
                          context={{ providerId: provider.id, providerSlug: provider.slug, serviceId: s.id, serviceSlug: s.slug, providerName: provider.displayName, serviceName: s.title, sourceType: "business_detail_service", sourceId: s.slug }}
                          label="咨询"
                          variant="secondary"
                        />
                      </div>
          ...
          <aside className="w-full lg:w-72 lg:flex-shrink-0">
            <div className="sticky top-4 space-y-4">
              ...
              <div className="rounded-xl border border-gray-200 bg-white p-4">
                <h3 className="mb-2 text-sm font-semibold text-gray-900">联系偏好</h3>
                <p className="text-xs text-gray-500">
                  {provider.contactPreference === "platform" && "仅通过平台消息联系"}
                  {provider.contactPreference === "email" && "接受邮件联系"}
                  {provider.contactPreference === "phone" && "接受电话联系（需先通过平台咨询）"}
                  {!provider.contactPreference && "通过平台咨询联系"}
                </p>
              </div>
            </div>
          </aside>

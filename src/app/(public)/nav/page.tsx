...
export default async function NavPage() {
  ...
  const linksByCategory = await prisma.category.findMany({
  ...
  const totalLinks = await prisma.linkItem.count({ where: { status: "active" } });
  ...
  const sidebarCategoryData = sidebarCategories.map(c => ({
    id: c.id,
    name: c.name,
    icon: c.icon || undefined,
        ...
        <div className="max-w-6xl mx-auto px-4 -mt-4">
          ...
          <div className="space-y-4 pb-10">
            {linksByCategory.map((category) => {
              ...
              const linkData = category.links.map(link => ({
                id: link.id,
                title: link.title,

...
export default function DestinationsIndexClient({ countries, featured }: DestinationsIndexClientProps) {
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3">
            {featured.map((c) => (
              <Link
                ...
                className="flex flex-col items-center gap-1 p-3 rounded-lg border border-purple-100 bg-white hover:bg-purple-50 hover:border-purple-300 transition-all text-center"
              >
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 rounded-lg border border-gray-200 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-400"
            />
          </div>
          <select
            value={regionFilter}
            onChange={(e) => setRegionFilter(e.target.value)}
            className="px-4 py-2.5 rounded-lg border border-gray-200 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-400 min-w-[120px]"
          >
              ...
              {items
                ...
                .map((c) => (
                  <Link
                    key={c.slug}
                    href={`/destinations/${c.slug}`}
                    className="group flex items-center gap-2 p-2.5 rounded-lg border border-gray-100 bg-white hover:shadow-sm hover:border-purple-200 transition-all"
                  >
                    <span className="text-xl flex-shrink-0">{c.flagEmoji}</span>
                    <div className="min-w-0 flex-1">
                      <div className="text-sm font-medium text-gray-900 truncate group-hover:text-purple-700 transition-colors">
                        {c.nameZh}
                      </div>
                    ...
                    {c.completenessTier === 1 && (
                      <span className="text-[10px] px-1.5 py-0.5 bg-purple-100 text-purple-700 rounded flex-shrink-0">完整</span>
                    )}

...
import { ArrowLeft, Printer, Download, Image, Crown, Plus, Trash2, ChevronDown, Eye, Palette, AlertTriangle, Settings, QrCode, Barcode, Save, Package, FileText, Loader2, Building2 } from 'lucide-react';
...
export default function LabelMakerPage() {
  ...
  // ── Company info globalization (read from bxb_company_profile — shared with document/invoice tools) ──
  const [companyInfo, setCompanyInfo] = useState<{ name: string; nameEn: string; address: string; phone: string; email: string; logo: string | null }>({ name: '', nameEn: '', address: '', phone: '', email: '', logo: null });

  useEffect(() => {
    // Load from the SAME key as document tools use (src/lib/documents/storage.ts)
    try {
      const raw = localStorage.getItem('bxb_company_profile');
      if (raw) {
        const cp = JSON.parse(raw);
        if (cp?.companyName) {
          setCompanyInfo({
            name: cp.companyName || '',
            nameEn: cp.companyNameEn || '',
            address: cp.address || '',
            phone: cp.phone || '',
            email: cp.email || '',
  ...
  // Sync formData with company info if not already set
  useEffect(() => {
    if (companyInfo.name && !formData.companyName) {
      setFormData(prev => ({
        ...prev,
        companyName: companyInfo.name,
        ...(companyInfo.logo ? { logoUrl: companyInfo.logo } : {}),
      }));
    }
  }, [companyInfo.name]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  // PNG export: v1.32.26 — captures from live previewRef, bypasses A4 tempDiv entirely
  const handleExportPNG = useCallback(async () => {
    setExporting(true);
    try {
      if (!previewRef.current) throw new Error('Preview element not found');

      // Temporarily remove any transforms that interfere with html2canvas
      const origTransform = previewRef.current.style.transform;
      const origOverflow = previewRef.current.style.overflow;
      previewRef.current.style.transform = 'none';
      previewRef.current.style.overflow = 'visible';

      const html2canvas = (await import('html2canvas')).default;

      // Capture directly from the visible preview — no hidden tempDiv, no A4 wrapper
      const canvas = await html2canvas(previewRef.current, {
        ...
        // Use the element's actual dimensions, NOT hardcoded A4 size
      ...
      // Restore styles
      previewRef.current.style.transform = origTransform;
      previewRef.current.style.overflow = origOverflow;

      const now = new Date();
      const ts = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-${String(now.getDate()).padStart(2, '0')}_${String(now.getHours()).padStart(2, '0')}${String(now.getMinutes()).padStart(2, '0')}${String(now.getSeconds()).padStart(2, '0')}`;
      const link = document.createElement('a');
      link.download = `label-${labelType?.titleEn || 'label'}_${size.width}x${size.height}mm_${ts}.png`;
  ...
  }, [selectedType, labelType, formData, style, template, size]);
  ...
  return (
    <>
      {/* Print CSS: native print- classnames + dynamic @page size injection */}
      <style jsx global>{`
        @media print {
          ...
          [data-label-preview] {
            ...
            width: ${size.width}mm !important; max-width: ${size.width}mm !important;
          ...
          .label-page-item { break-after: page !important; page-break-after: always !important; }
        ...
        <div className="max-w-7xl mx-auto px-4 py-4">
          ...
          <div className="flex gap-4 print-block">
            ...
            <div className={`w-full lg:w-5/12 no-print space-y-4 ${mobileTab === 'edit' ? 'block' : 'hidden lg:block'}`}>
              ...
              {/* Company Info (globalized from document tools) */}
              {companyInfo.name && (
                <div className="bg-white rounded-xl border p-4">
                  <h3 className="font-semibold text-gray-900 mb-3 flex items-center gap-2"><Building2 className="w-4 h-4 text-teal-600" /> 公司信息（自动同步）</h3>
                  <div className="flex items-center gap-3">
                    {companyInfo.logo && <img src={companyInfo.logo} alt="Logo" className="w-10 h-10 rounded object-cover shrink-0" />}
                    <div>
                      <p className="font-medium text-gray-900 text-sm">{companyInfo.name}</p>
                      {companyInfo.nameEn && <p className="text-xs text-gray-400">{companyInfo.nameEn}</p>}
                      {companyInfo.phone && <p className="text-xs text-gray-400">{companyInfo.phone}</p>}
                      {companyInfo.email && <p className="text-xs text-gray-400">{companyInfo.email}</p>}
                      <p className="text-xs text-gray-400 mt-1">来自发票/单据页 · 可在单据中心修改</p>
                    </div>
                  </div>
                </div>
              )}
            ...
            <div className={`w-full lg:w-7/12 print-full-width ${(mobileTab === 'preview' || mobileTab === 'export') ? 'block' : 'hidden lg:block'}`}>
              <div className="lg:sticky lg:top-20">
                ...
                {/* Label preview — v1.32.25: batch pagination, dynamic sizing, fixed logo layout */}
                <div ref={previewRef} data-label-preview="true"
                  ...
                  {/* Batch items or single label */}
                  {(batchItems.length > 0 ? batchItems : [{}]).map((batchItem, batchIdx) => (
                    <div key={batchIdx} className="label-page-item">
                      {/* Unified label container with proper header for Logo + Title */}
                      <div style={{ border: '1px solid #e5e7eb', borderRadius: '8px', padding: '12px', marginBottom: batchIdx > 0 ? '16px' : '0' }}>
                        {/* Header: Logo + Label Title — side by side */}
                        {(companyInfo.logo || formData.logoUrl) && (
                          <div className="flex items-center gap-2 mb-3">
                            <img src={formData.logoUrl || companyInfo.logo || ''} alt="Logo" className="h-6 w-auto object-contain shrink-0" />
                            <span className="text-xs text-gray-400">{labelType?.titleZh}</span>
                          </div>
                        )}
                        {renderLabelPreview(
                          selectedType,
                          { ...formData, ...(batchItem.cartonNo ? { cartonNo: batchItem.cartonNo } : {}) },
                          style,
                          labelType?.titleZh || '',
                          labelType?.titleEn || '',
                        )}

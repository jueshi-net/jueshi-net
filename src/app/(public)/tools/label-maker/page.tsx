...
import { ArrowLeft, Printer, Download, Image, Crown, Plus, Trash2, ChevronDown, Eye, Palette, AlertTriangle, Settings, QrCode, Barcode, Save, Package } from 'lucide-react';
...
export default function LabelMakerPage() {
  ...
  const handleSaveDraft = useCallback(() => {
    const saved = saveLabelDraft({
      type: selectedType,
      title: labelType?.titleZh || selectedType,
  ...
  const sizeOptions = labelSizes.filter(s => {
    if (s.memberOnly && !canUseCustomLabelSize()) return false;
    return true;
  });
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b sticky top-0 z-40 print:hidden">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={handleExportPNG} className="flex items-center gap-1.5 px-3 py-2 text-sm bg-green-600 text-white hover:bg-green-700 rounded-lg">
      ...
      <div className="max-w-7xl mx-auto px-4 py-4">
        ...
        <div className="flex gap-4 print:block">
          {/* Left: Controls */}
          <div className="w-full lg:w-5/12 print:hidden space-y-4">
            {/* Template selector */}
                    ...
                    <input type="file" accept="image/*" className="w-full text-sm"
                      onChange={e => {
                        ...
                        if (file) {
                          ...
                          reader.onload = ev => updateField('logoUrl', ev.target?.result);
            ...
            {canBatchGenerateLabels() && (
              <div className="bg-white rounded-xl border p-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button onClick={() => setShowBatch(!showBatch)} className="text-xs text-blue-600">
                    {showBatch ? '收起' : '展开'}
                  </button>
                </div>
                {showBatch && (
          ...
          {/* Right: Preview */}
          <div className={`w-full lg:w-7/12 print:w-full ${showPreview ? 'block' : 'hidden lg:block'}`}>
            <div className="lg:sticky lg:top-20">
              <div className="flex items-center justify-between mb-3 print:hidden">
                <h2 className="font-semibold text-gray-700 flex items-center gap-2"><Eye className="w-4 h-4" /> 实时预览</h2>
                <button onClick={() => setShowPreview(!showPreview)} className="text-xs text-gray-400">{showPreview ? '收起' : '展开'}</button>
              </div>

              {showPreview && (
                <div ref={previewRef} className="bg-white border rounded-xl p-6 print:p-0 print:border-0 print:shadow-none" style={{ fontFamily: style.showBorder ? 'sans-serif' : 'sans-serif' }}>
                  {/* Label content based on type */}

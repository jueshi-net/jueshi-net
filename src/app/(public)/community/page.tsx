...
export default async function CommunityPage({
  ...
  // ---- Parallel queries ----
  const [categories, posts, tagData, contributorData] = await Promise.all([
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6">
      {/* ===== Hero ===== */}
      <div className="rounded-2xl bg-gradient-to-br from-teal-600 via-teal-600 to-cyan-700 px-6 py-7 text-white mb-5">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          ...
          <Link
            href={newPostHref}
            className="inline-flex items-center gap-1.5 px-4 py-2.5 bg-white text-teal-700 rounded-xl text-sm font-semibold hover:bg-teal-50 transition flex-shrink-0 shadow-sm"
          >
      ...
      {/* ===== Categories (compact) ===== */}
      {categories.length > 0 && (
        <div className="flex items-center gap-2 overflow-x-auto pb-2 mb-5 -mx-1 px-1">
          <Link
            href="/community"
            className={`flex-shrink-0 px-3 py-1.5 rounded-lg text-sm font-medium border transition ${
              activeFilter === "latest"
          ...
          {categories.map((cat) => (
            <Link
              ...
              className="flex-shrink-0 inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-sm font-medium border border-gray-200 bg-white text-gray-600 hover:border-teal-300 hover:text-teal-700 transition"
            >

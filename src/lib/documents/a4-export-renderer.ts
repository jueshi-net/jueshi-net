...
export function buildA4ExportHTML(data: A4ExportData): string {
  ...
  if (data.companyLogo) inner += `<img src="${data.companyLogo}" alt="Logo" style="height:40px; margin-bottom:6px; object-fit:contain;" />`;
  ...
  // ===================== TERMS =====================
  if (data.defaultTerms || data.terms) {
    inner += `<div style="margin-bottom:18px;">`;
    inner += `<div style="font-size:12px; font-weight:bold; color:${pc}; margin-bottom:8px; padding-bottom:4px; border-bottom:1px solid ${safeBg(hbg)};">默认条款</div>`;
    inner += `<div style="font-size:11px; color:#334155; white-space:pre-wrap; line-height:1.6;">${esc(data.defaultTerms || data.terms)}</div></div>`;
  }
  if (data.remarks) {
    inner += `<div style="margin-bottom:18px;">`;
    inner += `<div style="font-size:12px; font-weight:bold; color:${pc}; margin-bottom:8px; padding-bottom:4px; border-bottom:1px solid ${safeBg(hbg)};">附加备注</div>`;
    inner += `<div style="font-size:11px; color:#334155; white-space:pre-wrap; line-height:1.6;">${esc(data.remarks)}</div></div>`;
  }
  ...
  // ===================== FOOTER =====================
  // Watermark removed per v1.14 — branding no longer rendered on exported documents

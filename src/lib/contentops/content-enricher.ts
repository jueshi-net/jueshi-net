...
export function enrichContent(input: EnricherInput): EnrichedOutput {
  ...
  // FAQ handling: keep model FAQ, or generate minimum for checklist/topic
  let resolvedFaq = faq || [];
  if (resolvedFaq.length < 3 && (contentType === 'checklist' || contentType === 'topic')) {
    // Generate minimum FAQ if model didn't provide enough
    if (contentType === 'checklist') {
      resolvedFaq = [
        {
      ...
      // Topic FAQ is generated by normalizer from content structure.
      // This is a fallback in case normalizer didn't generate enough.
      const topicTitle = resolvedTitle || topic || '';
      resolvedFaq = [
        {
          question: `这个专题涵盖了哪些主要内容？`,
          answer: `本专题为您提供关于${topicTitle || '相关主题'}的全面信息，包括工具推荐、操作指南、检查清单和官方资源。`
        },
        {
          question: `这个专题适合哪些人？`,
          answer: `本专题适合对${topicTitle || '相关内容'}感兴趣的海外华人和留学生，无论您是初学者还是有一定经验，都能从中获得有价值的信息。`
        },
        {
          question: `如何获取最新信息？`,

...
import {
  Home, Target, Crown, FileText, Building2, Heart, Settings,
  ArrowLeft,
} from 'lucide-react';
...
export function UserNavSidebar({ className }: { className?: string }) {
  ...
  return (
    <>
      <aside className={`${className ?? 'hidden md:flex'} flex-col w-60 bg-white border-r border-gray-100/80 h-full p-4 flex-shrink-0`}>
        <Link href="/" className="flex items-center gap-1.5 text-xs text-gray-400 hover:text-gray-600 mb-6 transition-colors">
          ...
          {NAV_ITEMS.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all duration-200 ${isActive ? `${theme.bg} ${theme.text} font-semibold shadow-sm` : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'}`}
              >
        ...
        <div className="mt-auto pt-4 border-t border-gray-100">
          <p className="text-[10px] text-gray-300 text-center">海外百宝箱 v1.20.42.6.3</p>
        </div>
      </aside>
      <MobileStickyTabs />
    </>
          ...
          {NAV_ITEMS.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`snap-start flex-shrink-0 flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-medium transition-all min-w-fit ${isActive ? `${theme.bg} ${theme.text} shadow-sm` : 'text-gray-500 hover:bg-gray-50 hover:text-gray-700'}`}
              >

...
(async () => {
  ...
  for (const page of pages) {
    ...
    try {
      ...
      console.log(`✓ ${page.path} (desktop): ${results[page.name].status}`);
      if (results[page.name].status === 'FAIL') {
        console.log(`  Reason: ${results[page.name].reason}`);
      }
      
    } catch (error) {
      console.error(`✗ ${page.path} (desktop): ERROR - ${error.message}`);
  ...
  // Summary
  console.log('\n=== SUMMARY ===');
  console.log(`Analytics (desktop): ${results['Analytics']?.status || 'NOT TESTED'}`);
  console.log(`Business (desktop): ${results['Business']?.status || 'NOT TESTED'}`);
  
  const allPass = 
    results['Analytics']?.status === 'PASS' &&
    results['Business']?.status === 'PASS';
  
  console.log(`\nOverall: ${allPass ? 'BATCH_03_PLAYWRIGHT_PASS' : 'BATCH_03_PLAYWRIGHT_FAILED'}`);

...
if (!data.heroSection || typeof data.heroSection !== "object") {
  ...
  // 8. sections 至少 2 个
  if (!Array.isArray(hs.sections) || hs.sections.length < 2) {
    errors.push(`sections 至少需要 2 个，当前 ${Array.isArray(hs.sections) ? hs.sections.length : 0} 个`);
  }

  // 9. checklistItems 总数至少 8 个
  const totalItems = Array.isArray(hs.sections)
    ? hs.sections.reduce((sum, s) => sum + (Array.isArray(s.items) ? s.items.length : 0), 0)
    : 0;
  if (totalItems < 8) {
    errors.push(`checklistItems 总数至少 8 个，当前 ${totalItems} 个`);
  }
  ...
  // 11. avoidPitfalls 至少 3 条
  if (!Array.isArray(hs.avoidPitfalls) || hs.avoidPitfalls.length < 3) {
    warnings.push(`avoidPitfalls 建议至少 3 条，当前 ${Array.isArray(hs.avoidPitfalls) ? hs.avoidPitfalls.length : 0} 条`);
  }
...
if (!Array.isArray(data.faqItems) || data.faqItems.length < 3) {
  errors.push(`faqItems 至少需要 3 个，当前 ${Array.isArray(data.faqItems) ? data.faqItems.length : 0} 个`);
} else {
...
// 17. relatedTools 至少 2 个
if (!Array.isArray(data.relatedTools) || data.relatedTools.length < 2) {
  errors.push(`relatedTools 至少需要 2 个，当前 ${Array.isArray(data.relatedTools) ? data.relatedTools.length : 0} 个`);
}

// 18. officialLinks 可以是占位但必须存在
if (!Array.isArray(data.officialLinks) || data.officialLinks.length === 0) {
  warnings.push("officialLinks 建议至少保留占位（URL 可为空）");
} else {
  const missingUrls = data.officialLinks.filter(l => !l.url || l.url === "");
  if (missingUrls.length > 0) {
    warnings.push(`${missingUrls.length} 个官方链接 URL 为空，需人工填写`);
  }
}

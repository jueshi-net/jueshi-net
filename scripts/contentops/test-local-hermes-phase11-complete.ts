#!/usr/bin/env tsx
/**
 * PHASE 11: Complete reference rewrite test
 * 
 * Test complete reference rewrite with full schema
 */
...
async function runCompleteTest() {
  ...
  console.log('=== PHASE 11: Complete reference rewrite test ===\n');
...
很多刚到海外的人都会下载一些常用 APP，例如地图导航、翻译、打车、外卖、银行、租房、二手交易、社交和本地生活服务。不同国家常用的软件不一样，比如北美常用 Google Maps、Uber、DoorDash、Facebook Marketplace，英国和澳洲也会用本地银行、交通和租房平台。留学生刚出国时，最容易遇到的问题是不会找路线、不知道怎么买电话卡、不熟悉当地支付方式、找房信息真假难辨、不了解二手交易风险。建议出国前先把常用 APP 下载好，并准备好邮箱、手机号、支付方式和身份验证材料。不同 APP 涉及隐私和银行卡信息，使用时要注意官方渠道下载，不要随便点陌生链接。对于刚到海外的用户，最好按照"出行、支付、住宿、吃饭、学习、社交、安全"几个场景来整理自己的手机工具箱。`;
  ...
  try {
    console.log('Calling local Hermes Agent with complete reference rewrite...');
    const startTime = Date.now();
    
    const result = await analyzeAndPlanLocal(TEST_INPUT, 'reference_rewrite');
    ...
    if (allPassed) {
      ...
      console.log('✅ Ready for PHASE 12');
    } else {
      console.log('❌ PHASE 11 FAILED');
      console.log('❌ Some validations failed');
    }

    // Save result for PHASE 12
    const fs = await import('fs');
    fs.writeFileSync('/tmp/phase11-result.json', JSON.stringify(result, null, 2));
    console.log('\nResult saved to /tmp/phase11-result.json for PHASE 12');

    return allPassed;
  } catch (error: any) {
    console.error('\n❌ PHASE 11 ERROR:', error.message);

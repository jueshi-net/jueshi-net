...
async function main() {
  console.log("═══════════════════════════════════════");
  console.log("🚀 商业级 HS-CIQ 终极灌浆启动");
  console.log("═══════════════════════════════════════");

  const startTime = Date.now();

  console.log("📖 读取商业级 JSON 数据...");
  const data = JSON.parse(readFileSync("/home/deploy/xixiong-saas/data/hs_codes_ready.json", "utf-8"));
  ...
  console.log(`\n✅ 终极灌浆完成！共灌入 ${inserted.toLocaleString()} 条，耗时 ${elapsed} 秒`);
